...
def build_feature_bank(feature_dir: str | Path, output_dir: str | Path) -> tuple[np.ndarray, list[dict]]:
    feature_dir = Path(feature_dir)
    output_dir = Path(output_dir)
    metadata_path = feature_dir / "metadata.jsonl"
    ...
    for idx, pooled_path in enumerate(pooled_paths, start=1):
        ...
        record = {
            ...
            "token_feature_path": record.get("token_feature_path"),
            "feature_shape": record.get("feature_shape", {"pooled": list(feature.shape)}),
        }
...
def main() -> None:
    args = parse_args()
    build_feature_bank(args.feature_dir, args.output_dir)

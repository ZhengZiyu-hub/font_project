...
def write_gallery(results: list[dict], output_path: str | Path, max_queries: int) -> None:
...
<head>
  <meta charset="utf-8" />
  <title>Qwen-VL Crop Retrieval Gallery</title>
  <style>
...
<body>
  <h1>Qwen-VL Crop Retrieval Gallery</h1>
  <div class="note">Showing {min(max_queries, len(results))} query crops. Each row: query + Top-10 similar crops.</div>
  {''.join(rows)}
</body>
</html>
"""
